...
def saveGroupsToSpreadsheet(groups, columns, filename, *, path=None, 
    ...
    if echo: printWorkbook(load_workbook(destination))
...
def printWorkbook(workbook):
    """
    Prints all values in `workbook`.
    
    Parameters:
        workbook (openpyxl.Workbook): the workbook whose values to print
    """
    for worksheet in workbook.worksheets:
        print("Cells of {}:".format(worksheet.title))
        for i, row in enumerate(worksheet, start=1):
            print("{:3}:".format(i), end="")
            for cell in row:
                value = cell.value if cell.value is not None else ""
                print(" {:10.10} |".format(value), end="")
            print("") # newline
